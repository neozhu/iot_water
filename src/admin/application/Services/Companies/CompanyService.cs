...
namespace WebApp.Services
{
  public class CompanyService : Service<Company>, ICompanyService
  {
    ...
    {
      var mapping = await this.mappingservice.Queryable().Where(x => x.EntitySetName == "Company" && ( ( x.IsEnabled == true ) || ( x.IsEnabled == false && !( x.DefaultValue == null || x.DefaultValue.Equals(string.Empty) ) ) )).ToListAsync();
    ...
    public async Task<Stream> ExportExcelAsync(string filterRules = "", string sort = "Id", string order = "asc")
    {
      ...
      var companies = await this.Query(new CompanyQuery().Withfilter(filters)).OrderBy(n => n.OrderBy(sort, order)).SelectAsync() ;
      ...
      return await ExcelHelper.ExportExcelAsync(typeof(Company), datarows);
