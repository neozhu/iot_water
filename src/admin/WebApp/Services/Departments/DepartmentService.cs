...
namespace WebApp.Services
{
  public class DepartmentService : Service<Department>, IDepartmentService
  {
    ...
    {
      var mapping = await _mappingservice.Queryable().Where(x => x.EntitySetName == "Department" && x.IsEnabled == true).ToListAsync();
    ...
    public async Task<Stream> ExportExcelAsync(string filterRules = "", string sort = "Id", string order = "asc")
    {
      ...
      var departments = await this.Query(new DepartmentQuery().Withfilter(filters)).Include(p => p.Company).OrderBy(n => n.OrderBy(sort, order)).SelectAsync();
      ...
      return await ExcelHelper.ExportExcelAsync(typeof(Department), datarows);
