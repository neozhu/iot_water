...
{
/// <summary>
/// File: AlarmLogQuery.cs
/// Purpose: easyui datagrid filter query 
/// Created Date: 3/29/2020 5:57:09 PM
/// Author: neo.zhu
/// Tools: SmartCode MVC5 Scaffolder for Visual Studio 2017
/// Copyright (c) 2012-2018 All Rights Reserved
/// </summary>
   public class AlarmLogQuery:QueryObject<AlarmLog>
   {
		public AlarmLogQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
               foreach (var rule in filters)
               {
						...
						if (rule.field == "InitDateTime" && !string.IsNullOrEmpty(rule.value) )
						{	
							if (rule.op == "between")
                            {
							    ...
							    And(x => SqlFunctions.DateDiff("d", start, x.InitDateTime) >= 0);
                                And(x => SqlFunctions.DateDiff("d", end, x.InitDateTime) <= 0);
						    }
						}
						if (rule.field == "DeviceId"  && !string.IsNullOrEmpty(rule.value))
						{
						...
						if (rule.field == "BeginDateTime" && !string.IsNullOrEmpty(rule.value) )
						{	
							if (rule.op == "between")
                            {
                                var datearray = rule.value.Split(new char[] { '-' });
							    ...
							    And(x => SqlFunctions.DateDiff("d", start, x.BeginDateTime) >= 0);
                                And(x => SqlFunctions.DateDiff("d", end, x.BeginDateTime) <= 0);
						    }
						}
						...
						{	
							if (rule.op == "between")
                            {
							    ...
							    And(x => SqlFunctions.DateDiff("d", start, x.CompletedDateTime) >= 0);
                                And(x => SqlFunctions.DateDiff("d", end, x.CompletedDateTime) <= 0);
						    }
						}
