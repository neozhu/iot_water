...
namespace WebApp
{
  public class ExcelHelper
  {
    ...
    public static Task<MemoryStream> ExportExcelAsync<T>(Type modelType, List<T> list, params string[] ignoredColumns) =>
      Task.Run(() =>
    {
      ExcelPackage.LicenseContext = OfficeOpenXml.LicenseContext.Commercial;
      var Heading = "";
      ...
      using (var pck = new ExcelPackage())
      {
        ...
        foreach (var col in props)
        {
          ...
          {
            if (fieldname.IndexOf("time", StringComparison.OrdinalIgnoreCase) >= 0)
            {
              columnCells.Style.Numberformat.Format = "yyyy-mm-dd hh:mm";
            }
            else
            {
