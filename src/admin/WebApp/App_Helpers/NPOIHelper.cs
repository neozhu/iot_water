...
namespace WebApp
{
  ...
  public class NPOIHelper
  {
    ...
    public static Task<MemoryStream> ExportExcelAsync<T>(string name, IEnumerable<T> list, Models.ExpColumnOpts[] colopts = null) => Task.Run(() => {
      ...
      foreach (var item in list)
      {
        ...
        foreach (var prop in outputproperties)
        {
          ...
          if (fieldtype == typeof(decimal) || fieldtype == typeof(Nullable<decimal>))
          {
            ...
            style.DataFormat = format.GetFormat("#,##0.000");
    ...
    public static Task<MemoryStream> ExportExcelAsync<T>(Type modelType, List<T> list, Models.ExpColumnOpts[] colopts) => Task.Run(() => {
      //var ignoredColumns = colopts.Where(x => x.IgnoredColumn == true);
      //var columns= colopts.Where(x => x.IgnoredColumn == false);
      var stream = new MemoryStream();
      var workbook = new XSSFWorkbook();
      ...
      var sheet = workbook.CreateSheet(modelType.Name);
      var headerRow = sheet.CreateRow(0);
      //Below loop is create header
      var headstyle = workbook.CreateCellStyle();
      var font = workbook.CreateFont();
      font.FontHeightInPoints = 11;
      font.Boldweight = (int)NPOI.SS.UserModel.FontBoldWeight.Bold;
      headstyle.SetFont(font);
      headstyle.BorderLeft = NPOI.SS.UserModel.BorderStyle.Thin;
      headstyle.BorderTop = NPOI.SS.UserModel.BorderStyle.Thin;
      headstyle.BorderRight = NPOI.SS.UserModel.BorderStyle.Thin;
      ...
      for (var i = 0; i < PropertyInfos.Length; i++)
      {
        ...
        var displayname = colopts.Where(x=>x.FieldName==fieldname).First().SourceFieldName;
        var cell = headerRow.CreateCell(col++);
